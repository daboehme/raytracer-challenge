...
mod tests {
    ...
    #[test]
    fn make_point() {
        let p = linalg::V4::make_point(1.0, -3.5, 9.0);
        assert_eq!(p.x(), 1.0);
        assert_eq!(p.y(), -3.5);
        assert_eq!(p.z(), 9.0);
    }
    ...
    #[test]
    fn add_pv() {
        let p = linalg::V4::make_point(4.0, -5.5, 10.0);
        let v = linalg::V4::make_vector(1.5, 2.0, 3.5);
        let r = linalg::V4::add(&p, &v);
        assert_eq!(r.x(), 5.5);
        assert_eq!(r.y(), -3.5);
        assert_eq!(r.z(), 13.5);
    }
    ...
    #[test]
    fn magnitude() {
        assert_eq!(linalg::V4::make_vector(1.0, 0.0, 0.0).magnitude(), 1.0);
        assert_eq!(linalg::V4::make_vector(-1.0, -2.0, -3.0).magnitude(), (14.0_f32).sqrt());
    }

    #[test]
    fn normalize() {
        let v1n = linalg::V4::make_vector(4.0, 0.0, 0.0).normalize();
        assert_eq!(v1n.x(), 1.0);
        assert_eq!(v1n.y(), 0.0);
        assert_eq!(v1n.z(), 0.0);
    }
    ...
    #[test]
    fn cross() {
        let a = linalg::V4::make_vector(1.0, 2.0, 3.0);
        let b = linalg::V4::make_vector(2.0, 3.0, 4.0);
        let c = linalg::V4::cross(&a, &b);
        assert_eq!(c.x(), -1.0);
        assert_eq!(c.y(), 2.0);
        assert_eq!(c.z(), -1.0);
        let c = linalg::V4::cross(&b, &a);
        assert_eq!(c.x(), 1.0);
        assert_eq!(c.y(), -2.0);
        assert_eq!(c.z(), 1.0)
    }
